...
export default function PresaleGridCard({ presale }: { presale: PresaleType }) {
  return (
    <Card className="hover:translate-y-[-30px] transition-all duration-300 rounded-lg cursor-pointer ">
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex space-x-2 items-center">
          ...
          <span className="uppercase text-xs">Finalized</span>
        </div>
        <div className="space-x-1">
          {presale.tags.map((tag, index) => (
            <Button
              key={index}
              size="tag"
              className="bg-gray-700 text-green-500 text-sm hover:bg-gray-700 hover:border hover:border-green-200 duration-200 transition-all"
            >
              {tag}
            </Button>
          ))}
          ...
          <div className="w-full ">
            ...
            <div className="font-bold text-xs">
              ...
              <p className="text-gray-400">
                Max Buy: {presale.maxBuy.toLocaleString("en-US")} USDT
              </p>
            </div>
          </div>
        </div>
        <div>
          ...
          <div className="flex items-center w-full flex-col">
            ...
            <div className="flex items-center justify-between w-full text-xs mt-1">
              <small>{presale.raised.toLocaleString("en-US")} USDT</small>
              <small>{presale.progress}%</small>
              <small>{presale.target.toLocaleString("en-US")} USDT</small>
            </div>
          </div>
            ...
            <div>
              <div className="flex items-center space-x-2 border-b border-gray-600 pb-1">
                ...
                <span className="text-green-500">
                  {presale.softcap.toLocaleString("en-US")} USDT
                </span>
              </div>
              <div className="flex w-full justify-around">
                <span>Rate</span>{" "}
                <span className="text-green-500">{presale.rate}</span>
              </div>
            </div>
            <div>
              <div className="flex items-center space-x-2 border-b border-gray-600 pb-1">
                <span>Hardcap</span>
                <span className="text-green-500">{presale.hardcap} USDT</span>
              </div>
              <div className="flex w-full justify-around">
                <span>Lock</span>{" "}
                <span className="text-green-500">{presale.lock}</span>
              </div>
            </div>
          </div>
        </div>
